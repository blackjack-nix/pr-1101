...
<?php
	...
	//}
	//$user = fgets($fichier_user); //on stoque dans une variable la ligne correspondant au hash de l'utilisateur
	...
	//$user_saisis = hash_redef($_GET["user"]); //On calcul le hash avec une fonction qui choist un hash disponible sur la machine
	//$mdp_saisis = hash_redef($_GET["mdp"]); //On calcul le hash avec une fonction qui choist un hash disponible sur la machine

	//$user_ok = hash_equals($user,$user_saisis);
	//$mdp_ok = hash_equals($mdp,$mdp_saisis);

	$user = "jekill";
	$mdp = "congrat_10";
	$user_saisis = $_GET["user"];
	$mdp_saisis = $_GET["mdp"];

	// ## Partie Authentification ##

	if (($user!=$user_saisis)||($mdp!=$mdp_saisis))	//on verifie si le hash en mémoire est bien le meme que celui rentré par l'utilisateur, ainsi que le mot de passe 
	{
